...
**Methods
    ...
    "! <p class="shorttext synchronized" lang="en"></p>
    "!
    "! @parameter p_deliverydocument | <p class="shorttext synchronized" lang="en"></p>
    "! @parameter p_plant | <p class="shorttext synchronized" lang="en"></p>
    "! @parameter r_XML | <p class="shorttext synchronized" lang="en"></p>
    METHODS xmldata
      IMPORTING p_deliverydocument TYPE i_deliverydocument-deliverydocument
...
CLASS ZCL_COA_PU IMPLEMENTATION.
...
*****************************POST METHOD*****************************
    ...
    DATA(xmldata) = me->xmldata(
                      p_deliverydocument = p_deliverydocument
                      p_plant            = p_plant
                    ).
    ...
    IF objectid IS NOT INITIAL AND xmldata IS NOT INITIAL AND r_token IS NOT INITIAL.
      ...
      CASE sy-sysid.
        WHEN lv_dev.
          DATA(ls_body) = VALUE struct(  xdp_template = 'COAPU_Dev/' && |{ objectid }|
                                               xml_data  = xmldata ).
        WHEN lv_qas.
          ls_body = VALUE struct(  xdp_template = 'COAPU_Dev/' && |{ objectid }|
                                               xml_data  = xmldata ).
        WHEN lv_prd.
          ls_body = VALUE struct(  xdp_template = 'COAPU_Prd/' && |{ objectid }|
                                               xml_data  = xmldata ).
      ENDCASE.
  ...
  METHOD get_obj.
    ...
    CASE sy-sysid.
      WHEN lv_dev.
        lo_request->set_uri_path( i_uri_path = '/v1/forms/COAPU_Dev' ).
      WHEN lv_qas.
        lo_request->set_uri_path( i_uri_path = '/v1/forms/COAPU_Dev' ).
      WHEN lv_prd.
        lo_request->set_uri_path( i_uri_path = '/v1/forms/COAPU_Prd' ).
    ENDCASE.
...
**Inspection Table
    ...
    DATA:specificationno    TYPE string,
         industrialname(40) TYPE c.
    ...
    TYPES: BEGIN OF ty_delivery_final,
             productdesc(200)         TYPE c,
             it_finalinspectionresult LIKE it_finalinspectionresult,
           END OF ty_delivery_final.


    DATA: it_delivery_final TYPE STANDARD TABLE OF ty_delivery_final,
          wa_delivery_final TYPE ty_delivery_final.

**********************************************************************
*****Delivery Table Logic

    SELECT DISTINCT deliverydocument,deliverydocumentitemtext,i_deliverydocumentitem~product,
    i_deliverydocumentitem~referencesddocument,deliverydocumentitem,
    i_deliverydocumentitem~inspectionlot,i_deliverydocumentitem~batch,i_deliverydocumentitem~actualdeliveryquantity,
...
LEFT JOIN i_batch            ON i_batch~material            = i_deliverydocumentitem~material
    ...
    WHERE i_deliverydocumentitem~deliverydocument   = @p_deliverydocument
    AND i_deliverydocumentitem~plant = @p_plant
    AND ( i_deliverydocumentitem~HigherLvlItmOfBatSpltItm = 000000 )
...
LEFT  JOIN i_customer         ON i_customer~customer                = i_deliverydocument~soldtoparty
    WHERE i_deliverydocument~deliverydocument = @p_deliverydocument
    AND i_deliverydocumentitem~plant          = @p_plant
    INTO TABLE @DATA(it_deliverydoc).

*select *
*FROM I_DELIVERYDOCUMENTITEM
*where  DeliveryDocument = @p_deliverydocument
*into table @data(lt_sdate).

*********************************************** added (20-01-2025)
SELECT from I_DELIVERYDOCUMENTITEM as i INNER JOIN ztableforcoa as z ON i~Plant = z~plant
 FIELDS DISTINCT z~plant,z~formatno,z~refsopno,z~revno,z~validfrom,z~validto,z~last_changed_at,z~local_created_at, z~local_created_by,z~local_last_changed_at,z~local_last_changed_by
  WHERE i~DeliveryDocument = @p_deliverydocument "and z~validfrom GE i~InspectionLotCreatedOn
...
data(lv_createdon) = value #( it_deliverydoc[ DeliveryDocument = p_deliverydocument ]-CreationDate OPTIONAL ).
...
DELETE it_inspection1 WHERE formatno NE lv_formatno .
    ...
    SORT it_deliverydoc BY product.
    SORT it_deliverymaintab BY product.
...
*****Inspection Lot Table
    ...
    IF it_deliverydoc IS NOT INITIAL.
      SELECT inspectionlot,deliverydocument,plant,billofoperationsgroup,batch,material,inspectionlotstartdate,inspectionlotcreatedontime
      FROM i_inspectionlot
      FOR ALL ENTRIES IN @it_deliverydoc
      WHERE material = @it_deliverydoc-product
      AND batch = @it_deliverydoc-batch
      AND plant = @p_plant
      INTO TABLE @DATA(it_inspection).
    ENDIF.

    SORT it_inspection BY inspectionlotstartdate DESCENDING inspectionlotcreatedontime DESCENDING.

    IF it_inspection IS NOT INITIAL.
      SELECT inspectionplangroup, replacedbillofoperations, inspectionplaninternalversion
      FROM i_inspectionplanversiontp_2
      FOR ALL ENTRIES IN @it_inspection
      WHERE inspectionplangroup = @it_inspection-billofoperationsgroup
      INTO TABLE @DATA(specificationtable).

      SORT specificationtable BY inspectionplangroup inspectionplaninternalversion.
    ENDIF.


    LOOP AT specificationtable INTO DATA(wa_specificationtable).
      CONCATENATE specificationno wa_specificationtable-replacedbillofoperations INTO specificationno SEPARATED BY ','.
    ENDLOOP.

    SHIFT specificationno RIGHT DELETING TRAILING ','.

    IF it_inspection IS NOT INITIAL.
      SELECT *
      FROM zi_qm_coa
      FOR ALL ENTRIES IN @it_inspection
      WHERE inspectionlot = @it_inspection-inspectionlot
      AND ( inspspecinformationfield1 = 'X' OR inspspecinformationfield1 = 'x' )
      INTO TABLE @DATA(inspectionresult).
    ENDIF.

    IF inspectionresult[] IS INITIAL.
      IF it_inspection[] IS NOT INITIAL.
        SELECT *
        FROM zi_qm_coa
        FOR ALL ENTRIES IN @it_inspection
        WHERE inspectionlot = @it_inspection-inspectionlot
        INTO TABLE @inspectionresult.
      ENDIF.
    ENDIF.

    SORT inspectionresult BY chars.

    LOOP AT it_deliverydoc INTO DATA(wa_deliveryitem).

      SELECT SINGLE industrystandardname
      FROM i_product
      WHERE product = @wa_deliveryitem-product
      INTO @industrialname.

      IF industrialname IS INITIAL.
        industrialname = wa_deliveryitem-deliverydocumentitemtext.
      ENDIF.

      CONCATENATE industrialname wa_deliveryitem-batch INTO wa_delivery_final-productdesc
      SEPARATED BY cl_abap_char_utilities=>cr_lf.


      READ TABLE it_inspection INTO DATA(wa_inspectionlot) WITH KEY material = wa_deliveryitem-product
                                                                    batch    = wa_deliveryitem-batch.

      IF sy-subrc EQ 0.
        LOOP AT inspectionresult INTO DATA(wa_inspectionresult) WHERE inspectionlot = wa_inspectionlot-inspectionlot.
        ...
        wa_delivery_final-it_finalinspectionresult[] = it_finalinspectionresult[].
        APPEND wa_delivery_final TO it_delivery_final.
        CLEAR: wa_delivery_final,
               count,
               it_finalinspectionresult[].
    ...
    CALL TRANSFORMATION ztr_coapu      SOURCE it_deliverydoc           = it_deliverydoc[]
...
*                                              revisiontabforcoa        = revisiontabforcoa
                                              ...
                                              it_delivery_final        = it_delivery_final[]
